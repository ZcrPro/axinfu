...
public class UnlockView extends View {
    ...
    private int normalR = 10;//radius of small circles;//
    private int selectR = 30;//radius of big circles;
    private int strokeWidth = 2;//width of big circles;
    private int normalColor = Color.parseColor("#a9d5ff");//defalt color of normal state
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        cirNorPaint.setStyle(Paint.Style.STROKE);
        cirNorPaint.setStrokeWidth(strokeWidth);
    ...
    private void drawCircles(Circle circle) {
        switch (circle.getState()) {
            case CIRCLE_NORMAL:
                mCanvas.drawCircle(circle.getX(), circle.getY(), selectR, cirNorPaint);
                break;
    ...
    private Circle getOuterCircle(int x, int y) {
        for (int i = 0; i < circleList.size(); i++) {
            ...
            if ((x - circle.getX()) * (x - circle.getX()) + (y - circle.getY()) * (y - circle.getY()) <= selectR * selectR) {
                if (circle.getState() != CIRCLE_SELECTED && isEnable) {
                    return circle;
                }
            }
        }
