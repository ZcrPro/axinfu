...
/**
 * Created by Vincent on 2017/4/27.
 */

public class PasswordInputEdtView extends EditText {
...
//    private boolean isBgFill;
    ...
    public void setOnInputOverListener(PasswordInputEdtView.onInputOverListener onInputOverListener) {
        this.onInputOverListener = onInputOverListener;
    }

    public PasswordInputEdtView(Context context) {
    ...
    public PasswordInputEdtView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public PasswordInputEdtView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//    }
    ...
    public void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        Log.i("wbq",text+","+start+","+lengthBefore+","+lengthAfter);
    ...
    private void setAttr(AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.PasswordInputEdtView, defStyleAttr, 0);
        isPwd = a.getBoolean(R.styleable.PasswordInputEdtView_isPwd, true);
        isAutoCloseKeyBoard = a.getBoolean(R.styleable.PasswordInputEdtView_autoCloseKeyBoard, true);
        isNumber = a.getBoolean(R.styleable.PasswordInputEdtView_isNumber, true);
        widthSpace = a.getDimensionPixelSize(R.styleable.PasswordInputEdtView_widthSpace, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics()));
        pwdType_CircleRadius = a.getDimensionPixelSize(R.styleable.PasswordInputEdtView_circleRadius, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics()));
        heightSpace = a.getDimensionPixelSize(R.styleable.PasswordInputEdtView_heightSpace, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 5, getResources().getDisplayMetrics()));
        rectStroke = a.getDimensionPixelSize(R.styleable.PasswordInputEdtView_rectStroke, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 2, getResources().getDisplayMetrics()));
        txtSize = a.getDimensionPixelSize(R.styleable.PasswordInputEdtView_txtSize, (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 18, getResources().getDisplayMetrics()));
        //isBgFill = a.getBoolean(R.styleable.PasswordInputEdtView_bgFill, false);
        numLength = a.getInt(R.styleable.PasswordInputEdtView_numLength, 6);
        textColor = a.getColor(R.styleable.PasswordInputEdtView_textColor, 0xff666666);
        rectNormalColor = a.getColor(R.styleable.PasswordInputEdtView_rectNormalColor, 0xffffffff);
        rectChooseColor = a.getColor(R.styleable.PasswordInputEdtView_rectChooseColor, 0xffffffff);
        pwdType = a.getInt(R.styleable.PasswordInputEdtView_pwdType, 0) == 0 ? PwdType.CIRCLE : PwdType.XINGHAO;
        isShowStock = a.getBoolean(R.styleable.PasswordInputEdtView_showStock, false);
...
//        }
        ...
        for (int i = 0; i < numLength; i++) {
            ...
            canvas.drawRoundRect(rectF,13,13,rectPaint);
            //canvas.drawRect(rect, rectPaint);
            if(isShowStock){
                rectPaint.setStyle(Paint.Style.STROKE);
                rectPaint.setColor(Color.rgb(217,238,245));
                Rect rect1 = new Rect(i * width + widthSpace, heightSpace, i * width + width - widthSpace, width - heightSpace);
                RectF rectF1 = new RectF(rect1);
                canvas.drawRoundRect(rectF1,13,13,rectPaint);
            }
