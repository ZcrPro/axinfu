...
import com.axinfu.modellib.thrift.bankcard.UPCardType;
...
public class CashierWayAdapter extends RecyclerAdapter<PayMethod> {
    ...
    public CashierWayAdapter(Context context, @Nullable List<PayMethod> data, @NonNull int... layoutResIds) {
     ...
     * @param layoutResIds
     */
    public CashierWayAdapter(Context context, Float amount, PayFeeRequestData payFeeRequestData, @Nullable List<PayMethod> data, @NonNull int... layoutResIds) {
        super(context, data, layoutResIds);
    ...
    public CashierWayAdapter(Context context, Float amount, PayECardRequestData payECardRequestData, @Nullable List<PayMethod> data, @NonNull int... layoutResIds) {
        super(context, data, layoutResIds);
        this.amount = amount;
        this.context = context;
    ...
    protected void convert(final RecyclerAdapterHelper helper, final PayMethod item) {
        ...
        if (item.channel.equals("UnionPay")) {
            ...
            ((TextView) helper.getView(R.id.promotion_id)).setTextColor(Color.parseColor("#208af0"));
            helper.setText(R.id.text, "银联在线支付");
            helper.setImageResource(R.id.img_way, R.mipmap.shanfu);
            if (!com.zhihuianxin.axutil.Util.isEmpty(item.assistance_hint)) {
                helper.setText(R.id.assistance_id, item.assistance_hint);
            }
        ...
        if (item.channel.equals("GuiYangCreditLoanPay")) {
            ...
            if (!com.zhihuianxin.axutil.Util.isEmpty(item.assistance_hint)) {
                ...
                helper.getView(R.id.assistance_id).setVisibility(View.GONE);
            }
            ...
            public void onClick(View v) {
                if (isFastDoubleClick()) {
                    ...
                    if (payFeeRequestData != null) {
                        payRequest.pay_for = com.xyaxf.axpay.modle.PayFor.SchoolFee;
                    ...
                    if (item.channel.equals("UnionPay")) {
                        if (Util.parseFloat(BankInfoCache.getInstance().getTradeLimit().trade_limit_amt, 0f) > 0.001f && amount > Float.parseFloat(BankInfoCache.getInstance().getTradeLimit().trade_limit_amt)) {
                            showPopwindow(1, amount, item);
                        } else {
                            payRequest.pay_method = item;
                            pay(payRequest);
                        }
                    } else if (item.channel.equals("RFID")) {
                        payRequest.pay_method = item;
                        getECardData(payRequest);
                    } else if (item.channel.equals("CCBWapPay")) {
                        payRequest.pay_method = item;
                        pay(payRequest);
                    } else if (item.channel.equals("QuickPay")) {
                        payRequest.pay_method = item;
                        //需要把支付方式一并传入
                        Bundle bundle = new Bundle();
                        QuickPayMethod quickPayMethod = new QuickPayMethod();
                        quickPayMethod.channel = item.channel;
                        quickPayMethod.assistance_hint = item.assistance_hint;
                        quickPayMethod.merchant_code = item.merchant_code;
                        quickPayMethod.supported_banks = item.supported_banks;
                        quickPayMethod.promotion_hint = item.promotion_hint;

                        bundle.putSerializable(QukMobileActivity.METHOD, quickPayMethod);
                        bundle.putFloat("amount", amount);
                        bundle.putSerializable("payRequest", payRequest);
                        Intent intent = new Intent(context, QukMobileActivity.class);
                        ...
                        payRequest.pay_method = item;
                        pay(payRequest);
                    } else if (item.channel.equals("UPTokenPay") && item.purpose != null) {
                        if (App.hasBankCard) {
                            ...
                            Intent intent = new Intent(context, UnionSweptEmptyCardActivity.class);
                            intent.putExtras(bundle);
                            context.startActivity(intent);
                        }
                    } else if (item.channel.equals("UPTokenPay") && item.purpose == null) {
                        ...
                        if (item.card.getCard_type().equals(UPCardType.Credit) || item.card.getCard_type().equals(UPCardType.QuasiCredit)) {
                            ...
                            PasswordWindow passwordWindow = new PasswordWindow(context);
                            ...
                            passwordWindow.showAtLocation(helper.getView(R.id.text), Gravity.BOTTOM, 0, 0);
                            passwordWindow.getPassword(new PasswordWindow.IGetpassword() {
                                ...
                                public void getpass(String password) {
                                    if (password != null) {
                                        ...
                                        upqrQuickPayChannelRequest.bank_card_code = item.card.getId();
                                        try {
...
//                                        payRequest.pay_method.card.setCard_no(item.card.getCard_no());
//                                        payRequest.pay_method.card.setIss_ins_name(item.card.getIss_ins_name());
//                                        payRequest.pay_method.card.setIss_ins_icon(item.card.getIss_ins_icon());
//                                        payRequest.pay_method.card.setIss_ins_code(item.card.getIss_ins_code());
//                                        payRequest.pay_method.card.setId(item.card.getId());
//                                        payRequest.pay_method.card.setCard_type_name(item.card.getCard_type_name());
                                        pay(payRequest);
                                    }
                                }
                            });
...
//                    GoPay.setEnabled(true);
        ...
        paymentService.payOder(NetUtils.getRequestParams(context, map), NetUtils.getSign(NetUtils.getRequestParams(context, map)))
                ...
                .subscribe(new BaseSubscriber<Object>(context, true, null) {
                    ...
                    public void onNext(Object o) {
                        ...
                        CashierWayAdapter.this.gyPayOderResponses = payOderResponse;
                        if (payOderResponse.resp.resp_code.equals(AppConstant.SUCCESS)) {
                            ...
                            it.putExtras(bundle);
                            try {
                                ((CashierDeskActivity) context).startActivityForResult(it, CashierDeskActivity.REQUEST_GR_PAY);
                            } catch (Exception e) {
    ...
    public void gysdkReturn(String tn) {
        ...
        Intent intent = new Intent(context, PaymentStatusActivity.class);
        Bundle bundle = new Bundle();
        bundle.putString(PaymentStatusActivity.AMOUNT, CashierWayAdapter.this.gyPayOderResponses.order.payment_amt);
        bundle.putString(PaymentStatusActivity.NAME, CashierWayAdapter.this.gyPayOderResponses.order.trade_summary + "-支付成功");

        PayMethod payMethod = new PayMethod();
        payMethod.channel = "GuiYangCreditLoanPay";

        bundle.putSerializable(PaymentStatusActivity.WAY, payMethod);
        bundle.putString(PaymentStatusActivity.TIME, CashierWayAdapter.this.gyPayOderResponses.order.order_time);
        bundle.putString(PaymentStatusActivity.ODER, CashierWayAdapter.this.gyPayOderResponses.order.order_no);
        intent.putExtras(bundle);
        context.startActivity(intent);
        ((Activity) context).finish();
