...
public class DownloadAppForceDialog extends Dialog {
    ...
    BroadcastReceiver GysdkReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if(!isShowing() && App.mAxLoginSp.getUpdateType().equals("Required"))show();
    ...
    View.OnClickListener clickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if(v.getId() == R.id.exit && !DownloadAPKService.isRunning()){
                MeFragment.logoutOperat();
                Intent intent = new Intent(MeFragment.BROADCAST_MEFRAGMENT_CLOSE);
                mContext.sendBroadcast(intent);
                mContext.startActivity(new Intent(mContext, LoginInputMobilActivityNew.class));
                dismiss();
            }
